...
export function DashboardContent({
  ...
  const currentUserDishes = userDishes.filter((ud) => ud.user_id === currentUser.id)
  ...
  const handleSubmitDishes = async () => {
    ...
    try {
      ...
      if (dishesToInsert.length > 0) {
        const { error } = await supabase.from("user_dishes").insert(dishesToInsert)
      ...
      toast({
        title: "Success!",
        description: "Your dish selections have been saved.",
      })
  ...
  const handleAddDish = async () => {
    ...
    try {
      ...
      toast({
        title: "Dish Added",
        description: `${newDish.name} has been added to the menu.`,
      })
  ...
  return (
    <div className="container py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-norway-red mb-2">Food Coordination Dashboard</h1>
        <p className="text-muted-foreground">Select dishes you'd like to bring to our 17th of May celebration</p>
      </div>
          ...
          <Card>
            <CardHeader className="bg-gradient-to-r from-norway-red to-norway-red/80 text-white">
              ...
              <CardDescription className="text-white/90">
                Choose what you'd like to bring to the celebration
              </CardDescription>
            </CardHeader>
                    ...
                    return (
                      ...
                      >
                        <Checkbox
                          id={`dish-${dish.id}`}
                          checked={isSelected}
                          onCheckedChange={(checked) => handleDishSelect(dish.id, checked as boolean)}
                          className="mt-1"
                        />
                        <div className="space-y-1 flex-1">
                          <div className="flex items-center justify-between">
                            <Label htmlFor={`dish-${dish.id}`} className="font-medium text-base cursor-pointer">
                              {dish.name}
                            </Label>
                          ...
                          {isSelected && dish.requires_partner && (
                            <div className="mt-3 pt-3 border-t">
                              <Label htmlFor={`partner-${dish.id}`} className="text-sm">
                                Partner's Name
                              </Label>
                              <Input
                                id={`partner-${dish.id}`}
                                placeholder="Who's helping you?"
                                className="mt-1"
                                value={partnerNames[dish.id] || ""}
                                onChange={(e) => handlePartnerNameChange(dish.id, e.target.value)}
                              />
                            </div>
                          )}
                        </div>
          ...
          {isAdmin && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Manage dishes for the 17th of May celebration</CardDescription>
              </CardHeader>
              <CardContent>
                <Dialog>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>Add a new dish to the menu for the 17th of May celebration.</DialogDescription>
                    </DialogHeader>
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setNewDish({ ...newDish, requires_partner: checked as boolean })
                          }
                        />
                        <Label htmlFor="requires-partner">Requires a partner to prepare</Label>
                      </div>
                    </div>

                    <DialogFooter>
                      <Button onClick={handleAddDish} className="bg-norway-red hover:bg-norway-red/90">
                        Add Dish
                      </Button>
                    </DialogFooter>
                  </DialogContent>
        ...
        <div>
          <Card>
            <CardHeader className="bg-gradient-to-r from-norway-blue to-norway-blue/80 text-white">
              ...
              <CardDescription className="text-white/90">See who's bringing what to the celebration</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <Tabs defaultValue="bringing">
                <TabsList className="w-full rounded-none">
                  <TabsTrigger value="bringing" className="flex-1">
                    Bringing Food ({usersWithDishes.length})
                  </TabsTrigger>
                  <TabsTrigger value="pending" className="flex-1">
                    Pending ({usersWithoutDishes.length})
                  </TabsTrigger>
                </TabsList>
                    ...
                    <div className="p-4">
                      {usersWithDishes.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-8 text-center">
                          ...
                          <p className="mt-2 text-muted-foreground">No participants have selected dishes yet.</p>
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {usersWithDishes.map((user) => {
                            const userSelectedDishes = userDishes.filter((ud) => ud.user_id === user.id)
                              ...
                              <div key={user.id} className="space-y-2">
                                ...
                                <ul className="ml-10 space-y-1">
                                  {userSelectedDishes.map((ud) => (
                                    <li key={ud.id} className="text-sm flex items-center text-muted-foreground">
                                      <span className="h-1.5 w-1.5 rounded-full bg-norway-red mr-2"></span>
                                      {ud.dish?.name}
                                      {ud.partner_name && (
                                        <span className="ml-1 text-xs">(with {ud.partner_name})</span>
                                      )}
                                    </li>
                                  ))}
                                </ul>
                    ...
                    <div className="p-4">
                      {usersWithoutDishes.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-8 text-center">
                          ...
                          <p className="mt-2 text-muted-foreground">Everyone has selected dishes!</p>
                        </div>
                      ) : (
